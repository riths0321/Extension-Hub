/**
 * popup.js — Main UI controller for Quick Link Manager Pro.
 * CSP-compliant: No innerHTML, no inline event handlers.
 */

import { StorageService } from '../services/storage.js';
import { TaggingService } from '../services/tagging.js';
import { SearchService } from '../services/search.js';
import { getDomain, getIcon, generateId, relativeTime, debounce, downloadFile, el, createSvg } from '../utils/helpers.js';
...
function safeGetElement(id) {
  ...
  if (!elem) console.warn('Element "' + id + '" not found');
  return elem;
}
...
const DOM = {
  ...
  btnClearSearch:      safeGetElement('btnClearSearch'),
  tagFilterRow:        safeGetElement('tagFilterRow'),
  favoritesSection:    safeGetElement('favoritesSection'),
  favoritesList:       safeGetElement('favoritesList'),
  allLinksSection:     safeGetElement('allLinksSection'),
  linksList:           safeGetElement('linksList'),
  sectionLabel:        safeGetElement('sectionLabel'),
  sectionCount:        safeGetElement('sectionCount'),
  emptyState:          safeGetElement('emptyState'),
  noResults:           safeGetElement('noResults'),
  btnSaveCurrent:      safeGetElement('btnSaveCurrent'),
  btnOpenAll:          safeGetElement('btnOpenAll'),
  btnExport:           safeGetElement('btnExport'),
  btnImport:           safeGetElement('btnImport'),
  btnArchive:          safeGetElement('btnArchive'),
  btnReadLater:        safeGetElement('btnReadLater'),
  btnHelp:             safeGetElement('btnHelp'),
  importFileInput:     safeGetElement('importFileInput'),
  sortByDate:          safeGetElement('sortByDate'),
  sortByLastOpened:    safeGetElement('sortByLastOpened'),
  saveModal:           safeGetElement('saveModal'),
  modalClose:          safeGetElement('modalClose'),
  modalTitle:          safeGetElement('modalTitle'),
  modalUrl:            safeGetElement('modalUrl'),
  modalTagsContainer:  safeGetElement('modalTagsContainer'),
  modalTagInput:       safeGetElement('modalTagInput'),
  modalAddTag:         safeGetElement('modalAddTag'),
  modalNote:           safeGetElement('modalNote'),
  modalCancel:         safeGetElement('modalCancel'),
  modalSave:           safeGetElement('modalSave'),
  tagSuggestions:      safeGetElement('tagSuggestions'),
  editModal:           safeGetElement('editModal'),
  editModalClose:      safeGetElement('editModalClose'),
  editTitle:           safeGetElement('editTitle'),
  editUrl:             safeGetElement('editUrl'),
  editTagsContainer:   safeGetElement('editTagsContainer'),
  editTagInput:        safeGetElement('editTagInput'),
  editAddTag:          safeGetElement('editAddTag'),
  editNote:            safeGetElement('editNote'),
  editModalCancel:     safeGetElement('editModalCancel'),
  editModalSave:       safeGetElement('editModalSave'),
  readLaterModal:      safeGetElement('readLaterModal'),
  readLaterModalClose: safeGetElement('readLaterModalClose'),
  readLaterList:       safeGetElement('readLaterList'),
  helpModal:           safeGetElement('helpModal'),
  helpModalClose:      safeGetElement('helpModalClose'),
  exportModal:         safeGetElement('exportModal'),
  exportModalClose:    safeGetElement('exportModalClose'),
  exportJSON:          safeGetElement('exportJSON'),
  exportMarkdown:      safeGetElement('exportMarkdown'),
  exportHTML:          safeGetElement('exportHTML'),
  archiveModal:        safeGetElement('archiveModal'),
  archiveModalClose:   safeGetElement('archiveModalClose'),
  archiveList:         safeGetElement('archiveList'),
  tabActive:           safeGetElement('tabActive'),
  tabArchived:         safeGetElement('tabArchived'),
  ...
  try {
    ...
    chrome.runtime.sendMessage({ action: 'getActiveTab' }, (res) => {
      if (res && res.tab && res.tab.url) {
        try { state.currentDomain = new URL(res.tab.url).hostname.replace('www.', ''); } catch (_) {}
      }
      render();
    });
    bindEvents();
    ...
    showToast('Something went wrong loading the extension');
  }
}
...
function updateSortButtons() {
  if (!DOM.sortByDate || !DOM.sortByLastOpened) return;
  DOM.sortByDate.classList.toggle('active', state.sortBy === 'date');
  DOM.sortByLastOpened.classList.toggle('active', state.sortBy === 'lastOpened');
}
...
function render() {
  ...
  let filtered = SearchService.filter(state.links, state.query, state.activeTags, state.currentDomain);
  filtered = getSortedLinks(filtered);

  const favorites     = state.links.filter((l) => l.favorite);
  const showFavorites = favorites.length > 0 && !state.query && state.activeTags.size === 0;

  if (DOM.favoritesSection && DOM.favoritesList) {
  ...
  if (DOM.noResults)       DOM.noResults.hidden        = !(!hasResults && isFiltering);
  if (DOM.allLinksSection) DOM.allLinksSection.hidden  = !hasResults;

  if (hasResults) {
    if (DOM.sectionLabel) DOM.sectionLabel.textContent = state.query ? 'Results' : (state.activeTags.size > 0 ? 'Filtered' : 'All Links');
...
function createCard(link) {
  ...
  // Hover preview
  let previewTimeout;
  card.addEventListener('mouseenter', () => { previewTimeout = setTimeout(() => showPreview(card, link), 500); });
  card.addEventListener('mouseleave', () => {
    clearTimeout(previewTimeout);
    const tooltip = card.querySelector('.link-preview-tooltip');
    if (tooltip) tooltip.remove();
  });

  // Click → open background tab
...
function showPreview(card, link) {
  const tooltip  = el('div', 'link-preview-tooltip');
...
async function saveLink() {
  ...
  if (result.duplicate) {
    ...
    await loadData(); closeSaveModal(); render();
...
async function saveEdit() {
  ...
  await loadData(); closeModal(DOM.editModal); render();
...
function renderReadLaterList() {
  ...
  if (!state.readLater.length) {
    ...
    const p = el('p', 'empty-title'); p.textContent = 'No links in queue';
    empty.appendChild(p); DOM.readLaterList.appendChild(empty); return;
  }
  state.readLater.forEach((link) => {
    ...
    const markBtn = el('button', 'read-later-mark'); markBtn.textContent = 'Mark Read';
    ...
    acts.appendChild(markBtn); item.appendChild(acts);
...
async function openArchiveModal() {
  ...
  await loadData();
  state.currentArchiveView = 'archived'; // Always open on archived tab
  syncArchiveTabs();
  renderArchiveList();
...
function renderArchiveList() {
  ...
  items.forEach((link) => {
    ...
    if (isArchived) {
      const restoreBtn = el('button', 'unarchive-btn'); restoreBtn.textContent = 'Restore';
      ...
      const delBtn = el('button', 'delete-permanent-btn'); delBtn.textContent = 'Delete';
      delBtn.addEventListener('click', async (e) => {
        e.stopPropagation();
        if (confirm('Permanently delete this archived link?')) {
      ...
      const archBtn = el('button', 'unarchive-btn'); archBtn.textContent = 'Archive';
...
function openModal(modal) {
  if (!modal) return;
  document.querySelectorAll('.modal-overlay.is-open').forEach((m) => { if (m !== modal) m.classList.remove('is-open'); });
...
function bindEvents() {
  ...
  if (DOM.btnOpenAll) {
    DOM.btnOpenAll.addEventListener('click', () => {
      let visible = getSortedLinks(SearchService.filter(state.links, state.query, state.activeTags));
  ...
  if (DOM.modalTagInput) {
    DOM.modalTagInput.addEventListener('keydown', (e) => { if (e.key === 'Enter') { e.preventDefault(); addModalTag(); } });
  }
  ...
  if (DOM.editTagInput) {
    DOM.editTagInput.addEventListener('keydown', (e) => { if (e.key === 'Enter') { e.preventDefault(); addEditTag(); } });
  }
...
function bindKeyboardShortcuts() {
  document.addEventListener('keydown', (e) => {
    ...
    if (e.key === '/' && document.activeElement !== DOM.searchInput) {
      e.preventDefault(); if (DOM.searchInput) DOM.searchInput.focus(); return;
    }
    if (e.key === 'ArrowDown' || e.key === 'ArrowUp') {
      navigateCards(e.key === 'ArrowDown' ? 1 : -1); return;
    }
...
function navigateCards(dir) {
  ...
  const next = dir === 1 ? (cards[idx + 1] || cards[0]) : (cards[idx - 1] || cards[cards.length - 1]);
